...
public class Edl {
  ...
  static void constMap() {
    ...
    // Sert à décaler le code au cas où l'unité "programme" n'a pas de variable
    // globale et ne produit pas de réserver
    int decalageReserver = 0;
    if (nbVarGlob == 0) {
      decalageReserver = 2; // 2 instructions en plus (reserver + nb de param)
    }
  ...
  private static void unionDef(int descIndex) {
    if (tabDesc[descIndex].getNbDef() > MAXDEF) {
    ...
    for (int i = 1; i < tabDesc[descIndex].getNbDef() + 1; i++) {
      ...
      Descripteur.EltDef newEltDef = tabDesc[descIndex].new EltDef(tabDesc[descIndex].getDefNomProc(i),
          tabDesc[descIndex].getDefAdPo(i) + transCodeDecalage[descIndex], tabDesc[descIndex].getDefNbParam(i));
  ...
  public static void main(String argv[]) {
    ...
    for (int i = 0; i < nMod + 1; i++) {
      for (int k = 1; k < tabDesc[i].getNbRef() + 1; k++) {
        ...
        if (adFinale[i][k] == -1) {
          erreur(FATALE, "Procedure décrite en ref mais n'existe pas");
        }
      }
    }
